...
steps_and_sleep_summary <- steps_and_sleep %>%
  group_by(time_only) %>%
  summarize(mean_opportunity_score = mean(opportunity_score, na.rm = TRUE))

# Mean opportunity score by time interval
ggplot(steps_and_sleep_summary, aes(x = time_only, y = mean_opportunity_score)) +
  geom_col(fill = "darkorange") +
...
```{r}
library(ggcorrplot)
correlation_matrix <- cor(steps_and_sleep[, c("avg_steps", "avg_sleep", "avg_calories","avg_intensity","avg_mets")])
ggcorrplot(correlation_matrix, hc.order = TRUE, type = "lower", lab = TRUE)
```
**Correlation Analysis: The correlation matrix shows that there is a strong positive correlation between steps and calories burned, steps and intensity, and steps and METs. There is a weak positive correlation between steps and sleep. \**

# Boxplot
```{r}
ggplot(intervals, aes(x = factor(hour), y = opportunity_score)) +
  geom_boxplot(fill = "lightblue", outlier.color = "red") +
  labs(title = "Distribution of Opportunity Scores by Hour",
       x = "Hour of the Day (24-hour format)",
       y = "Opportunity Score") +
  theme_minimal()
```
**Shows median opportunity score while showing variability (outliers)**
