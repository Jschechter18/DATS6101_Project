...
hourly_summary_df <- intervals_df %>%
  ...
  summarize(mean_opportunity_score = mean(opportunity_score, na.rm = TRUE),
            ...
            mean_norm_sleep = mean(norm_sleep, na.rm = TRUE)
            )
...
hourly_summary_df <- hourly_summary_df %>%
  mutate(
    opportunity_level = cut(mean_opportunity_score,
                            breaks = quantile(mean_opportunity_score, probs = c(0, 1/3, 2/3, 1), na.rm = TRUE),
                            labels = c("bad", "mid", "good"),
                            include.lowest = TRUE)
  )
```
...
ggplot(hourly_summary_df, aes(x = hour_military, y = mean_opportunity_score)) +
...
## QQ Plot Intensity
```{r}
qqnorm(minutes_summary_df$avg_intensity)
qqline(minutes_summary_df$avg_intensity, col = "red")
```


## QQ Plot Sleep
```{r}
qqnorm(minutes_summary_df$avg_sleep)
qqline(minutes_summary_df$avg_sleep, col = "red")
...
# Null Hypothesis: There is no difference in opportunity score between the different time intervals
# Alternative Hypothesis: There is a difference in opportunity score between the different time intervals
hourly_summary_df$mean_opportunity_score
kruskal.test(hourly_summary_df$mean_opportunity_score ~ hourly_summary_df$hour_military)
```
